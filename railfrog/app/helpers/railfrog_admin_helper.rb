module RailfrogAdminHelper
  ...
  def create_mapping_tree(mappings, parent_id=0)
    ...
    for map in mappings[parent_id] do
      tree += '<li>'
      tree += link_to_function('', visual_effect(:toggle_slide, "tree_#{map.id}", :duration => 0.5)+" if ($('link_tree_#{map.id}').hasClassName('expand')) { $('link_tree_#{map.id}').addClassName('collapse').removeClassName('expand'); } else { $('link_tree_#{map.id}').addClassName('expand').removeClassName('collapse'); }", :class => ( parent_id == 0 ? 'collapse' : 'expand' ), :id => "link_tree_#{map.id}") if !map.rgt.nil? && !map.lft.nil? && ((map.all_children.size > 0) || map.chunk_id.nil?)
    ...
    if parent_id == 0
      tree += javascript_tag("$('tree_#{SiteMapping.find_root.id}').show()") 
    else
      tree += javascript_tag("$('tree_#{parent_id}').hide()") 
    end
