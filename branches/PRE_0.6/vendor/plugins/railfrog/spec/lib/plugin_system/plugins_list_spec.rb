...
context "A new/empty PluginsList" do
  setup do
    @plugins_list = PluginSystem::PluginsList.new
  end
  ...
  specify "can add a plugin to itself" do
    plugin = PluginSystem::Plugin.new(File.join(@@__plugin_system_specs, 'another_plugin-0.0.1.gemspec'))
  ...
  specify "can add multiple plugins to itself" do
    ...
    plugins = specs.map do |spec|
      PluginSystem::Plugin.new(File.join(@@__plugin_system_specs, "#{spec}.gemspec"))
    end
...
context "A PluginsList with one plugin" do
  setup do
    @plugin = PluginSystem::Plugin.new(File.join(@@__plugin_system_specs, 'the_first_plugin-0.0.1.gemspec'))
    @plugins_list = PluginSystem::PluginsList.new(@plugin)
  end
...
context "A PluginsList with multiple plugins" do
  setup do
    ...
    @plugins_list = PluginSystem::PluginsList.new(*@plugins)
  end
...
context "A new/empty DynamicPluginsList" do
  setup do
    @dynamic_plugins_list = PluginSystem::DynamicPluginsList.new(PluginSystem::PluginsList.new, :some_filter)
  end
  ...
  specify "should be a kind of PluginsList" do
    @dynamic_plugins_list.should_be_a_kind_of PluginSystem::PluginsList
  end
...
context "A DynamicPluginsList that filters for enabled plugins with one enabled plugin" do
  setup do
    ...
    @dynamic_plugins_list = PluginSystem::DynamicPluginsList.new(PluginSystem::PluginsList.new(@plugin), :enabled?)
  end
