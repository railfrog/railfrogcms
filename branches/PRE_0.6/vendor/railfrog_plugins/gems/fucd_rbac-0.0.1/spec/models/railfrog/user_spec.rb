...
module Railfrog
  ...
  context "A saved user with an associated role" do
    ...
    specify "should have permission for action 'foobar'" do
      @role.stubs(:grants_permission_for?).with('foobar').returns(true)
      @user.has_permission_for?('foobar').should_be true
    end
    
    specify "should not have permission for action 'blah'" do
      @role.stubs(:grants_permission_for?).with('blah').returns(false)
      @user.has_permission_for?('blah').should_be false
    end
  end
end
